...
# ===========================================================================
# Custom CSS — adapts to Streamlit's chosen theme (light or dark).
#
# Strategy:
#   Use Streamlit's own CSS custom properties directly:
#     --background-color          → page background
#     --secondary-background-color → card / sidebar backgrounds
#     --text-color                → primary text
#   These automatically track the Streamlit theme toggle, so cards and
#   text always match the active theme with zero JavaScript needed.
#
#   For status colours (badges, borders) we use medium-intensity tones
#   that provide acceptable contrast on both light and dark backgrounds.
# ===========================================================================
...
/* ===================================================================
   Custom properties that sit on top of Streamlit's palette.
   Streamlit sets --background-color, --secondary-background-color,
   --text-color, and --font on :root.  Our card/hero elements use
   --secondary-background-color as their background so they always
   contrast slightly with the page.
   =================================================================== */
:root {
  /* Accent & glow */
  --umm-accent:       #3b82f6;
  --umm-accent-glow:  rgba(59, 130, 246, 0.12);

  /* Status colours — medium-intensity so they read on any background */
  --umm-healthy-bg:   rgba(34, 197, 94, 0.12);
  --umm-healthy-fg:   #22c55e;
  --umm-healthy-bd:   rgba(34, 197, 94, 0.35);

  --umm-warning-bg:   rgba(245, 158, 11, 0.12);
  --umm-warning-fg:   #f59e0b;
  --umm-warning-bd:   rgba(245, 158, 11, 0.35);

  --umm-critical-bg:  rgba(239, 68, 68, 0.12);
  --umm-critical-fg:  #ef4444;
  --umm-critical-bd:  rgba(239, 68, 68, 0.35);

  --umm-info-bg:      rgba(59, 130, 246, 0.12);
  --umm-info-fg:      #3b82f6;
  --umm-info-bd:      rgba(59, 130, 246, 0.35);
}
...
/* Preserve Material Symbols / Icons fonts — Streamlit uses them for
   icons (file uploader, navigation arrows, etc.).  Without this,
   the icon glyphs render as literal text like "upload". */
...
[data-testid="stFileUploaderDropzoneInstructions"] span[class*="material"],
span[class*="Icon"],
...
.metric-card {
    ...
    border-left:    4px solid var(--umm-accent);
    border-radius:  14px;
    padding:        20px 24px;
    margin-bottom:  10px;
...
.metric-card.healthy  { border-left-color: var(--umm-healthy-fg); }
.metric-card.warning  { border-left-color: var(--umm-warning-fg); }
.metric-card.critical { border-left-color: var(--umm-critical-fg); }
.metric-card .label {
    font-size: 0.72rem;
    opacity: 0.65;
...
/* ===================================================================
   Feature cards (Home page hero tiles)
   =================================================================== */
.feature-card {
    background:     var(--secondary-background-color);
    color:          var(--text-color);
...
.badge-healthy  { background: var(--umm-healthy-bg);  color: var(--umm-healthy-fg);  border: 1px solid var(--umm-healthy-bd); }
.badge-warning  { background: var(--umm-warning-bg);  color: var(--umm-warning-fg);  border: 1px solid var(--umm-warning-bd); }
.badge-critical { background: var(--umm-critical-bg); color: var(--umm-critical-fg); border: 1px solid var(--umm-critical-bd); }
.badge-info     { background: var(--umm-info-bg);     color: var(--umm-info-fg);     border: 1px solid var(--umm-info-bd); }
.badge-passed   { background: var(--umm-healthy-bg);  color: var(--umm-healthy-fg);  border: 1px solid var(--umm-healthy-bd); }
.badge-failed   { background: var(--umm-critical-bg); color: var(--umm-critical-fg); border: 1px solid var(--umm-critical-bd); }
...
.section-title {
    font-size: 1.05rem; font-weight: 700;
    border-bottom: 2px solid var(--umm-accent);
...
.hero::before {
    ...
    background: radial-gradient(ellipse at 30% 20%, var(--umm-accent-glow) 0%, transparent 70%);
    pointer-events: none;
}
.hero h1 {
...
/* Sidebar — clean professional spacing */
...
# Plotly defaults
# ===========================================================================
# Theme-neutral Plotly defaults:
#   - transparent backgrounds → chart sits on Streamlit's actual page color
#   - mid-grey font + semi-transparent grid → readable on light AND dark
#
# Use AXIS_STYLE for every xaxis/yaxis kwarg (replaces the old hardcoded
# `gridcolor=GRID_COLOR`).
GRID_COLOR  = "rgba(127,127,127,0.2)"
...
def page_dashboard() -> None:
    ...
    fig.update_layout(**CHART_LAYOUT, yaxis=dict(range=[0, 1.05], gridcolor=GRID_COLOR),
...
def page_ai_insights() -> None:
    ...
    if chosen_provider == PROVIDER_DISABLED:
        ...
        if preset.has_key:
            pass  # key is auto-loaded into the field below
        else:
...
def render_sidebar() -> str:
    with st.sidebar:
        st.markdown('<div class="sidebar-title">🧠 ML Model Monitor</div>', unsafe_allow_html=True)
        st.markdown("---")
        ...
        # --- Status card ---
        status_lines = []
        status_lines.append(f'<b>Mode:</b> {st.session_state.mode}')
        if st.session_state.reference_df is not None:
            status_lines.append(f'<b>Reference:</b> {st.session_state.reference_df.shape[0]:,} rows')
        if st.session_state.current_batches:
            status_lines.append(f'<b>Batches:</b> {len(st.session_state.current_batches)}')
        ...
        if summary is not None and not summary.empty:
            ...
            status_lines.append(f'<b>Health:</b> <span style="color:{color};font-weight:700">{health}</span>')

        st.markdown(
            '<div class="sidebar-status">' + '<br>'.join(status_lines) + '</div>',
            unsafe_allow_html=True,
        )
        ...
        st.markdown('<div class="sidebar-footer">All inference is local. No cloud APIs used.</div>',
                    unsafe_allow_html=True)
    return choice
